...
cargo build --release -p mpe-serve

# Then: solve + serve in one command.
./target/release/mpe-serve \
  --region london \
...
- **Coloured lines** — the visiting order of each route, straight-line
  ...
  `dijkstra::ch::query_with_path` for each consecutive pair, which is
...
These are the same datasets that mpe-cli was tested on, served live.
...
1. **Real polylines** — currently straight lines. To draw road-following
   geometry, call `dijkstra::ch::query_with_path_into` for each
