"""mpe_py + Flask — Python serves the HTTP layer, Rust does the solving.
...
mpe-serve rebuild. The Rust solver runs inside this same Python
process via `import mpe_py`, so the dataset lives in the same RAM
...
import mpe_py  # the Rust extension built by `maturin develop`


HERE = pathlib.Path(__file__).resolve().parent
INDEX_PATH = HERE / "index.html"
REPO_ROOT = HERE.parent.parent.parent  # crates/mpe-py/python -> repo root


def main() -> int:
    p = argparse.ArgumentParser(description="Run the mpe-engine Flask UI on top of the Rust solver.")
    ...
    # CORS open — same as mpe-serve, so a separate UI host can hit us too.
    @app.after_request
