...
#[derive(Parser, Debug)]
#[command(name = "mpe", version, about = "mpe-engine: routing + VRP in one process.")]
struct Cli {
    #[command(subcommand)]
    cmd: Cmd,
}
...
fn main() -> Result<()> {
    ...
    match cli.cmd {
        ...
        Cmd::Pipeline {
            ...
            let tmp = std::env::temp_dir().join(format!("mpe-pipeline-{}.json", seed));
...
fn cmd_solve(
    ...
    // 2. mmap-load the dijkstra caches (~20 µs each regardless of size).
    ...
    //    (dur + dist) at no extra Dijkstra cost.
...
fn collect_coords(
    ...
    if !problem.shipments.is_empty() {
        bail!("shipments not yet wired into mpe-cli — use brooom directly for now");
    }
