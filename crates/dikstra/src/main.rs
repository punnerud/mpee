...
fn run_suite(label: &str, g: CsrGraph, src: u32) {
    ...
    let (ref_dist, t_bin) = bench("dijkstra_binary", &|| dijkstra_binary(&g, src));
    let (d_4ary, t_4ary) = bench("dijkstra_4ary", &|| dijkstra_4ary(&g, src));
...
fn main() {
    println!("SSSP-benchmark - Dijkstra-varianter vs delta-stepping vs Duan-inspirert");
