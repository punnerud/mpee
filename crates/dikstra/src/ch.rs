...
///      upward-only Dijkstra in `graph_fwd` from `s` and store
///      `(s_idx, dist)` in every reached vertex's bucket.
///
///   2. **Backward sweep over destinations.** For each destination `t`,
///      run an upward-only Dijkstra in `graph_bwd` from `t`. At every
...
/// backward Dijkstra (cheap on CH, ~1 ms each). Bucket scans are unchanged
/// in total work.
...
/// CH p2p query: bidirectional Dijkstra som kun relaxer upward-kanter.
