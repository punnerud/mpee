...
//! our bidirectional Dijkstra and OSRM (if running on localhost:5002).

use std::time::Instant;

use sssp_bench::bidir::{bidir_dijkstra, transpose};
...
fn main() -> std::io::Result<()> {
    ...
    for &(s, d) in &pairs {
        bidir_results.push(bidir_dijkstra(g, &g_bwd, s, d).unwrap_or(f32::INFINITY));
    }
    let bidir_ms = t.elapsed().as_secs_f64() * 1000.0;
    println!(
        "bidir_dijkstra:      {:>9.1} ms total ({:.3} ms/query)",
        bidir_ms,
        bidir_ms / n_queries as f64
    );

    // ---- Reference: full Dijkstra on first 30 pairs ----
    let n_ref = 30usize;
    let t = Instant::now();
    ...
    println!(
        "full Dijkstra:       {:>9.0} ms for {} queries ({:.1} ms/query)",
    ...
    println!("\nCorrectness vs full Dijkstra (first {n_ref} pairs):");
    println!("  ch::query:       {}/{} OK", n_ref - bad_ch, n_ref);
    println!("  bidir_dijkstra:  {}/{} OK", n_ref - bad_bidir, n_ref);
    ...
    println!("\nSpeedup vs full Dijkstra (per query):");
    ...
    println!("Per-query:");
    println!("  full Dijkstra:  {:.1} ms", per_full);
