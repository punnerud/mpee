...
fn main() -> std::io::Result<()> {
    ...
    for (idx, &src) in sources.iter().enumerate() {
        ...
        let t = Instant::now();
        let d_4 = dijkstra_4ary(&g, src);
        let dt = t.elapsed().as_secs_f64() * 1000.0;
        t_4ary.add(dt);
        println!("  dijkstra_4ary:   {:>7.1} ms", dt);
        total_checks += 1;
    ...
    print_row("dijkstra_binary", &t_bin);
    print_row("dijkstra_4ary", &t_4ary);
    ...
    println!();
    println!(
        "  speedup (mean) vs dijkstra_binary:  4ary={:.2}x  dstep={:.2}x  duan={:.2}x  auto={:.2}x",
