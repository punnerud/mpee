//! Full comparison of OUR Dijeng stack vs petgraph (the Rust ecosystem).
//! Covers all benchmark workloads used elsewhere in the project.

use ordered_float::OrderedFloat;
// petgraph's external API is named after the algorithm (Edsger Dijkstra);
// only the source path stays `dijkstra` — our local alias keeps the brand.
use petgraph::algo::dijkstra as pg_dijeng;
...
fn run(label: &str, g: &CsrGraph, src: u32, skip_pg: bool) -> Row {
    ...
    println!("  our dijeng_binary:   {:>9.2} ms   ({reachable}/{n} reach)", t_bin);

    let t = Instant::now();
    let r4 = dijeng_4ary(g, src);
    let t_4 = t.elapsed().as_secs_f64() * 1000.0;
    let bad_4 = count_bad(&r, &r4);
    println!("  our dijeng_4ary:     {:>9.2} ms   {}", t_4,
             if bad_4 == 0 { "OK" } else { "FAIL" });

    let t = Instant::now();
    let r8 = dijeng_8ary(g, src);
    let t_8 = t.elapsed().as_secs_f64() * 1000.0;
    let bad_8 = count_bad(&r, &r8);
    println!("  our dijeng_8ary:     {:>9.2} ms   {}", t_8,
    ...
    if !skip_pg {
        ...
        let t = Instant::now();
        let pg_map = pg_dijeng(&pg, pg_nodes[src as usize], None, |e| *e.weight());
        t_pg = t.elapsed().as_secs_f64() * 1000.0;
        let pg_vec = pg_to_vec(&pg_map, n);
        bad_pg = count_bad(&r, &pg_vec);
        println!(
            "  petgraph (build={:>5.0} ms + dijeng): {:>9.2} ms   {}",
            build_ms,
            t_pg,
...
fn main() -> std::io::Result<()> {
    ...
    if std::path::Path::new("data/soc-LiveJournal1.txt.gz").exists() {
        match load_snap_edge_list("data/soc-LiveJournal1.txt.gz") {
            Ok((g, _)) => {
                // 137M edges -> petgraph build ~1-2s, dijeng ~3-5s. We accept it.
