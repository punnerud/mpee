...
fn main() -> std::io::Result<()> {
    ...
    println!("=== sekvensiell ===");
    let t = Instant::now();
    let (g_seq, c_seq, _ed_seq, _nm_seq, _np_seq) = load_osm_routing(&path, Profile::Car)?;
    ...
    println!("=== parallell ===");
    let t = Instant::now();
    let (g_par, c_par, _ed_par, _nm_par, _np_par) = load_osm_routing_par(&path, Profile::Car)?;
