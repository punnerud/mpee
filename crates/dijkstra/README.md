...
Correctness verified against full Dijkstra on every benchmark (ε = 1e-3 relative).
...
bidirectional Dijkstra that only relaxes "up-edges" finds the shortest
...
The algorithm:
1. **Forward sweep** — per src, an upward Dijkstra in `graph_fwd`,
   accumulating `(s_idx, dur, dist)` in a bucket per settled vertex u.
2. **Backward sweep** — per dst, an upward Dijkstra in `graph_bwd`, and
...
The algorithm is plain Dijkstra on the uncontracted reordered graph
