//! Two Dijkstra implementations.
//!
//! `dijkstra_binary` — textbook binary heap with lazy deletion (skip stale
//! entries on pop). Avoids the cost of supporting `decrease_key`.
//!
//! `dijkstra_4ary` — 4-ary heap. Shallower (log_4 n vs log_2 n levels), and
...
pub fn dijkstra_binary(g: &CsrGraph, src: u32) -> Vec<f32> {
    let n = g.n;
...
pub fn dijkstra_4ary(g: &CsrGraph, src: u32) -> Vec<f32> {
    let n = g.n;
    let mut dist = vec![INF; n];
    dist[src as usize] = 0.0;
    let mut heap: Vec<HeapItem> = Vec::with_capacity(n);
...
pub fn dijkstra_8ary(g: &CsrGraph, src: u32) -> Vec<f32> {
    let n = g.n;
    let mut dist = vec![INF; n];
    dist[src as usize] = 0.0;
    let mut heap: Vec<HeapItem> = Vec::with_capacity(n);
